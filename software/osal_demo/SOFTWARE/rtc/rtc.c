...
// Unix时间戳转换为日期时间
void unix_timestamp_to_datetime(uint32_t timestamp, _calendar_obj* dt) {
    ...
    // 计算月份和日期
    uint8_t month = 0;
    uint8_t* days_in_months = (uint8_t*)mon_table;
    ...
    dt->w_month = month + 1;  // 月份从1开始
    dt->w_date = days + 1;     // 日期从1开始
}
